...
function renderDashboard(data) {
  ...
  var dataCount = dc
    ...
    .html({
      some:
        '<strong>Transactions: </strong><strong>%filter-count</strong> selected out of <strong>%total-count</strong> records | <a href="javascript:dc.filterAll(); dc.renderAll();">Reset All</a>',
      all:
        "<strong>Transactions: </strong>All <strong>%total-count</strong> records selected. Please click on the graph to apply filters."
    });
  ...
  matrixMonth
    ...
    .on("pretransition", function(chart) {
      ...
      let yScale = d3
        .scaleBand()
        .domain(monthAmountGroup.all().map(d => months[Number(d.key)]))
        .range([ch, 0]);
      ...
      chart.selectAll("g.box-group text").remove();
...
// Get the modal
let modal = document.getElementById("userData");

// Get the button that opens the modal
let btnUserData = document.getElementById("btnUserData");
let btnSave = document.getElementById("btnSave");
let btnCancel = document.getElementById("btnCancel");

// When the user clicks on the button, open the modal
btnUserData.onclick = function() {
  modal.style.display = "block";
};

btnSave.onclick = function() {
  let taUserData = document.getElementById("taUserData");
  modal.style.display = "none";
  loadInputText(taUserData.value);
};
...
function loadInputText(textData) {
  //Category,SubCategory,Property,Amount,Year,Month

  let data = d3.csvParse(textData, function(d, i) {
    //console.log(d,i);
    d.Amount = +d.Amount;
    d.Year = +d.Year;
    d.MonthNumber = months
...
function refreshData(data) {
  ...
  ndx.add(data);
  matrixMonth.calculateColorDomain();
  matrixQtr.calculateColorDomain();
  dc.redrawAll();
}
